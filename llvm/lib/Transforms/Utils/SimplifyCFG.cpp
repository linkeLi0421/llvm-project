...
static int dmp = 0;
//linke
static irpb::IRFunctionBook* IR_func_book;
...
static void CloneInstructionsIntoPredecessorBlockAndUpdateSSAUses(
  ...
  for (Instruction &BonusInst : *BB) {
    ...
    if (PTI->getDebugLoc() != NewBonusInst->getDebugLoc()) {
      ...
      // dingzhu patch: this makes no sense.
    }
...
bool SimplifyCFGOpt::SpeculativelyExecuteBB(BranchInst *BI, BasicBlock *ThenBB,
  ...
  for (PHINode &PN : EndBB->phis()) {
    ...
    // dingzhu patch: new generated select should inherit branchinst's loc
    Instruction *I = dyn_cast<Instruction>(V);
...
static bool performBranchToCommonDestFolding(BranchInst *BI, BranchInst *PBI,
  ...
  // dingzhu patch: PBI share BI's destination, then inherit its loc
  DebugLoc DummyDl = PBI->getDebugLoc();
  PBI->setDebugLoc(BI->getDebugLoc());
  ...
  // dingzhu patch: as this or-op read PB's cmp result, it should inherit
  // PBI's original dl
  auto Lop = createLogicalOp(Builder, Opc, PBI->getCondition(), BICond, "or.cond");
  Instruction *LI = dyn_cast<Instruction>(Lop);
  LI->setDebugLoc(DummyDl);
  PBI->setCondition(Lop);
...
static bool passingValueIsAlwaysUndefined(Value *V, Instruction *I, bool PtrValueMayBeModified) {
  ...
  if (C->isNullValue() || isa<UndefValue>(C)) {
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(Use))
      if (!LI->isVolatile()) {
        // dingzhu patch: dump info
        bool res = !NullPointerIsDefined(LI->getFunction(),
                                     LI->getPointerAddressSpace());
        if (res && dmp) {
          dbgs() << "UB: Load from null:";
          I->dump();
          I->getDebugLoc().dump();
        }
        return res;
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(Use))
      if (!SI->isVolatile()) {
        bool res = (!NullPointerIsDefined(SI->getFunction(),
                                      SI->getPointerAddressSpace())) &&
               SI->getPointerOperand() == I;
        if (res && dmp) {
          dbgs() << "UB: Store to null:";
          I->dump();
          I->getDebugLoc().dump();
        }   
        return res;
        ...
        for (const llvm::Use &Arg : CB->args())
          if (Arg == I) {
            ...
            if (CB->isPassingUndefUB(ArgIdx) &&
              ...
              bool res = !PtrValueMayBeModified;
              if (res && dmp) {
                dbgs() << "UB: Passing null to a nonnnull+noundef argument:";
                I->dump();
                I->getDebugLoc().dump();
              }   
              return res;
        ...
        for (const llvm::Use &Arg : CB->args())
          if (Arg == I) {
            ...
            if (CB->isPassingUndefUB(ArgIdx)) {
              // Passing undef to a noundef argument is undefined.
              if (dmp) {
                dbgs() << "UB: Passing undef to a noundef argument:";
                I->dump();
                I->getDebugLoc().dump();
              }
              return true;
  ...
  for (PHINode &PHI : BB->phis())
    for (unsigned i = 0, e = PHI.getNumIncomingValues(); i != e; ++i)
      if (passingValueIsAlwaysUndefined(PHI.getIncomingValue(i), &PHI)) {
        std::string BBname = BB->hasName() ? BB->getName().str() : "no name bb";
        std::string Funcname = BB->getParent() ? (BB->getParent()->hasName()? BB->getParent()->getName().str() : "no name func") : "no name func";
        std::string modulename;
        if (BB->getParent()) {
          auto mo = BB->getParent()->getParent();
          if (mo) {
            modulename = mo->getName().str();
          }
        }
        dbgs() << "\n###module name:" << modulename << " funcname:" << Funcname << " undefbbname:" << BBname;
        BasicBlock *Predecessor = PHI.getIncomingBlock(i);
        std::string predBBname = Predecessor->hasName() ? Predecessor->getName().str() : "no name bb";
        dbgs() << " predbbname:" << predBBname << "\n";
        dbgs() << " pred branch:";

        Instruction *T = Predecessor->getTerminator();
        // dump Predecessor cmp inst branch inst toproto
        IRBuilder<> Builder(T);
        if (BranchInst *BI = dyn_cast<BranchInst>(T)) {
          ...
          if (BI->isUnconditional()) {
            ...
            dbgs() << "Conditional; ";
            dbgs() << "check: ";
            dyn_cast<Instruction>(T->getOperand(0))->dump();
...
bool SimplifyCFGOpt::simplifyOnce(BasicBlock *BB) {
  ...
  // if (BB->getParent() && BB->getParent()->getName() == "ntfs_collate_binary") {
  //   dbgs() << "###1\n";
  //   BB->getParent()->dump();
  // }

  // if (BB->getName() == "gem_abnormal_irq.exit") {
  //   dbgs() << "###start1" << '\n';
  //   BB->getParent()->dump();
  //   dbgs() << "###end\n";
  // }
  ...
  if (removeUndefIntroducingPredecessor(BB, DTU)) {
    ...
    //   }
    // }
    // dbgs() << "###module name:" << modulename << " funcname:" << Funcname << " bbname:" << BBname << "\n";
    return requestResimplify();
  }
  dmp = 0;
