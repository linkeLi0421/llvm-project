...
static bool iterativelySimplifyCFG(Function &F, const TargetTransformInfo &TTI,
  ...
  while (LocalChange) {
    ...
    for (Function::iterator BBIt = F.begin(); BBIt != F.end(); ) {
      ...
      // if (F.getName() == "finish_inode_if_needed") {
      //   for (auto &BBI : F) {
      //     if (BBI.getName() == "if.then60.i.i") {
      //       // if (dyn_cast<BranchInst>(BBI.getTerminator())->isUnconditional()) {
      //         dbgs() << "\n1get it\n";
      //         // BB->dump();
      //         BBI.dump();
      //         BB.dump();
