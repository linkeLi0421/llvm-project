...
void SSACCmpConv::convert(SmallVectorImpl<MachineBasicBlock *> &RemovedBlocks) {
  ...
  // dingzhu patch: new gen ccmp should inherit pred's br loc
  DebugLoc brLoc = Head->findBranchDebugLoc();
  ...
  if (CmpMI->getOperand(FirstOp + 1).isReg())
    MRI->constrainRegClass(CmpMI->getOperand(FirstOp + 1).getReg(),
                           TII->getRegClass(MCID, 1, TRI, *MF));
  // dingzhu patch: ccmp should inherit former cond-solver's loc
  MachineInstrBuilder MIB = BuildMI(*Head, CmpMI, brLoc, MCID)
