...
void SelectionDAGBuilder::visit(const Instruction &I) {
  ...
  CurInst = &I;
  // if (I.getParent() && I.getParent()->getName() == "land.rhs.i439") {
  //   dbgs() << "before visit";
  //   I.dump();
  //   I.getParent()->dump();
  //   DAG.dump();
...
SelectionDAGBuilder::EmitBranchForMergedCondition(const Value *Cond,
  ...
  // dingzhu patch: new br's loc should inherit BOp's user
  // if (CurBB->getName() == "if.end" && DAG.getMachineFunction().getName() == "crypto_ctr_create") {
  //   DAG.dump();
  //   // CurBB->dump();
  //   // BOp->getInstIndex()->dump();
  // }
  const Instruction *BOp = dyn_cast<Instruction>(Cond);
  const Instruction *brBop = nullptr;
  if (BOp != nullptr && !BOp->user_empty()) {
    // dbgs() << "###ok\n";
    // Cond->dump();
    // BOp->dump();
    // if (BOp->use_begin()->getUser())
      brBop = dyn_cast<Instruction>(BOp->use_begin()->getUser());
  }
  auto tmpLoc = getCurSDLoc();
  if (brBop != nullptr) {
    // brBop->dump();
    // brbop->getdebugloc().dump();
    tmpLoc = SDLoc(brBop, SDNodeOrder);
  }
  ...
  if (const CmpInst *BOp = dyn_cast<CmpInst>(Cond)) {
    ...
    if (CurBB == SwitchBB ||
      ...
      CaseBlock CB(Condition, BOp->getOperand(0), BOp->getOperand(1), nullptr,
                   TBB, FBB, CurBB, tmpLoc, TProb, FProb);
  ...
  CaseBlock CB(Opc, Cond, ConstantInt::getTrue(*DAG.getContext()),
               nullptr, TBB, FBB, CurBB, tmpLoc, TProb, FProb);
  SL->SwitchCases.push_back(CB);
}
...
void SelectionDAGBuilder::FindMergedConditions(const Value *Cond,
  ...
  // next level.
  Value *NotCond;
  // if (CurBB->getParent()->getName() == "inet6_dump_addr") {
  //   dbgs() << "###head of FMC\n";
  //   Cond->dump();
  //   CurBB->dump();
  //   SwitchBB->dump();
  ...
  // if (CurBB->getName() == "if.end61.i.i") {
  //   dbgs() << "###FMC1\n";
  //   Cond->dump();
  // }

  // if (CurBB->getName() == "land.rhs.i439") {
  //   dbgs() << "###cond FindMergedConditions";
  //   Cond->dump();
  // }
  ...
  if (Opc == Instruction::Or) {
    ...
    // if (CurBB->getName() == "if.end61.i.i") {
    //   dbgs() << "###or LHS FindMergedConditions\n";
    //   Cond->dump();
    //   TmpBB->dump();
    // }
    ...
    FindMergedConditions(BOpOp1, TBB, FBB, TmpBB, SwitchBB, Opc, Probs[0],
                         Probs[1], InvertCond);
    // if (CurBB->getName() == "if.end61.i.i") {
    //   dbgs() << "###or RHS FindMergedConditions\n";
    //   Cond->dump();
    //   TmpBB->dump();
    // }
  } else {
    ...
    FindMergedConditions(BOpOp0, TmpBB, FBB, CurBB, SwitchBB, Opc, NewTrueProb,
                         NewFalseProb, InvertCond);
    // if (CurBB->getName() == "if.end61.i.i") {
    //   dbgs() << "###and RHS FindMergedConditions\n";
    //   Cond->dump();
    //   TmpBB->dump();
    // }
...
void SelectionDAGBuilder::visitBr(const BranchInst &I) {
  ...
  // if (BrMBB->getName() == "if.end" && DAG.getMachineFunction().getName() == "crypto_ctr_create") {
  //   DAG.dump();
  // }
    ...
    if (Opcode && !(match(BOp0, m_ExtractElt(m_Value(Vec), m_Value())) &&
      ...
      if (ShouldEmitAsBranches(SL->SwitchCases)) {
        for (unsigned i = 1, e = SL->SwitchCases.size(); i != e; ++i) {
          // if (BrMBB->getName() == "if.end61.i.i") {
          //   dbgs() << "before export\n";
          //   SL->SwitchCases[i].CmpLHS->dump();
          //   SL->SwitchCases[i].CmpRHS->dump();
          //   SL->SwitchCases[0].ThisBB->dump();
          //   SL->SwitchCases[0].ThisBB->getBasicBlock()->dump();
          // }
          ExportFromCurrentBlock(SL->SwitchCases[i].CmpLHS);
          ExportFromCurrentBlock(SL->SwitchCases[i].CmpRHS);
          // dingzhu patch: last block should inherit ori br's loc
          if (i == e - 1) {
            auto oriDL = SL->SwitchCases[i].DL;
            SDLoc brDdl = SDLoc(dyn_cast<Instruction>(&I), oriDL.getIROrder());
            SL->SwitchCases[i].DL = brDdl;
          }
          // if (BrMBB->getName() == "if.end61.i.i") {
          //   dbgs() << "after export\n";
          //   SL->SwitchCases[i].CmpLHS->dump();
        ...
        // if (I.getParent() && I.getParent()->getName() == "if.end61.i.i") {
        //   dbgs() << "####before visitswitchcase\n";
        //   DAG.dump();
        // }
        // Emit the branch for this block.
        visitSwitchCase(SL->SwitchCases[0], BrMBB);
        // // dingzhu patch: new gen brcond should inherit select's loc.
        // //  may not deadly right
        // auto brNode = DAG.getRoot();

        // if (BrMBB->getName() == "if.then51" && DAG.getMachineFunction().getName() == "xhci_bus_suspend") {
        //   DAG.dump();
        //   BOp->dump();
        //   brNode->dump();
        // }
        // // if block doesn't split, no need to do this
        // std::set<InstIndex*> idxSet;
        // for (auto &I : *BrMBB) {
        //   idxSet.insert(I.getInstIndex());
        // }
        // if (brNode.getOpcode() == ISD::BR) {
        //   auto brccNode = brNode.getOperand(0);
        //   if (brccNode.getOpcode() == ISD::BRCOND || 
        //           brccNode.getOpcode() == ISD::BR_CC ) {
        //     InstIndex *cond0II = brccNode->getOperand(0)->getInstIndex();
        //     InstIndex *cond1II = brccNode->getOperand(1)->getInstIndex();
        //     InstIndex *BOpII = BOp->getInstIndex();
        //     if (BrMBB->getName() == "if.then" && DAG.getMachineFunction().getName() == "convert_extent_bit") {
        //       DAG.dump();
        //       BOp->dump();
        //       brNode->dump();
        //       brccNode->dump();
        //     }
        //     if (idxSet.find(cond0II) != idxSet.end() && idxSet.find(cond1II) != idxSet.end()) {
        //       if (BOp->getOpcode() == Instruction::Select) {
        //         brNode.getNode()->setInstIndex(BOpII);
        //         brccNode.getNode()->setInstIndex(BOp->getInstIndex());
        //         }
                
        //       // dingzhu patch: not only select, any cond-solver should be inherited
        //       //  && dyn_cast<Instruction>(BOp->getOperand(0))->getOpcode() == Instruction::ICmp
        //       if (BOp->getOpcode() == Instruction::Or) {
        //         auto brNode = DAG.getRoot();
        //         brNode.getNode()->setInstIndex(BOpII);
        //         brccNode.getNode()->setInstIndex(BOp->getInstIndex());
        //       }
  ...
  // Create a CaseBlock record representing this branch.
  // dingzhu patch: this process can do such transformation:
  //     select 
  //     br
  // Emit:
  //     brcond
  ...
  CaseBlock CB(ISD::SETEQ, CondVal, ConstantInt::getTrue(*DAG.getContext()),
               nullptr, Succ0MBB, Succ1MBB, BrMBB, sdl);
...
void SelectionDAGBuilder::visitSwitchCase(CaseBlock &CB,
  ...
  // if (SwitchBB->getName() == "if.end61.i.i") {
  //   dbgs() << "###visitswitchcase1\n";
  //   SwitchBB->dump();
  //   CB.ThisBB->dump();
  //   CB.ThisBB->getBasicBlock()->dump();
  //   DAG.dump();
  // }
