...
SDValue DAGCombiner::foldLogicOfSetCCs(bool IsAnd, SDValue N0, SDValue N1,
  ...
  if (OpVT != RL.getValueType())
    return SDValue();
  if (DAG.getMachineFunction().getName() == "rtnl_net_getid") {
    dbgs() << "###2";
    // N->dump();
    DAG.dump();
  }
  ...
  if (LR == RR && CC0 == CC1 && IsInteger) {
    ...
    if (AndEqZero || AndGtNeg1 || OrNeZero || OrLtZero) {
      // dingzhu patch: loc should be consistent(need dconfirm)
      SDValue Or = DAG.getNode(ISD::OR, SDLoc(DL), OpVT, LL, RL);
    ...
    if (AndEqNeg1 || AndLtZero || OrNeNeg1 || OrGtNeg1) {
      // dingzhu patch: loc should be consistent(need dconfirm)
      SDValue And = DAG.getNode(ISD::AND, SDLoc(DL), OpVT, LL, RL);
